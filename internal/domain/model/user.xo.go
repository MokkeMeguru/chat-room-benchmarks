...
func (u *User) Insert(ctx context.Context, db DB) error {
	...
	const sqlstr = `INSERT INTO public.users (` +
		...
		`)`
	// run
	logf(sqlstr, u.UserID, u.Name, u.CreatedAt, u.UpdatedAt)
	if _, err := db.ExecContext(ctx, sqlstr, u.UserID, u.Name, u.CreatedAt, u.UpdatedAt); err != nil {
		return logerror(err)
	}
	// set exists
	u._exists = true
...
func (u *User) Update(ctx context.Context, db DB) error {
	...
	const sqlstr = `UPDATE public.users SET ` +
		...
		`WHERE user_id = $4`
	// run
	logf(sqlstr, u.Name, u.CreatedAt, u.UpdatedAt, u.UserID)
	if _, err := db.ExecContext(ctx, sqlstr, u.Name, u.CreatedAt, u.UpdatedAt, u.UserID); err != nil {
		return logerror(err)
	}
	return nil
}
...
func UserByUserID(ctx context.Context, db DB, userID string) (*User, error) {
	...
	if err := db.QueryRowContext(ctx, sqlstr, userID).Scan(&u.UserID, &u.Name, &u.CreatedAt, &u.UpdatedAt); err != nil {
